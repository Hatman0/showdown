...
export const Moves: {[moveid: string]: MoveData} = {
	...
	explosion: {
		...
		basePowerCallback(source, target, move) {
			if (this.field.isTerrain('electricterrain')) {
				if (!source.isAlly(target)) this.hint(`${source.species.name}'s Explosion is boosted x1.5.`);
		...
		onEffectiveness(typeMod, target, type, move) {
			if (this.field.isTerrain('electricterrain')) {
				if (!source.isAlly(target)) this.hint(`${source.species.name}'s Explosion is electrified.`);
